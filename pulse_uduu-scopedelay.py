...
def setup_scope_uduu(scope, signal_channel, trigger_channel, max_voltage,
                     capture_width_ns, record_length, vdiv,
                     delay_seconds, verbose=False):
    """Setup oscilloscope with horizontal delay for UDUU"""
    ...
    # Calculate delay from trigger to U2 pulse start

    # Use edge trigger with horizontal delay
    scope.setup_edge_trigger(
        source=f'CH{trigger_channel}',
        level=0.45,
        slope='RISE',
        mode='NORMAL',
        delay=True,  # Enable horizontal delay
    )

    scope.horizontal_delay_time = delay_seconds
    ...
    if verbose:
        ...
        print(f"Horizontal delay: {delay_seconds*1e6:.3f} µs")
...
def run_uduu(pulse_amplitude, pulse_width_ns, u_to_d_delay, d_to_u_delay, u_to_u_delay,
    ...
    try:
        ...
        setup_pulse_channel(
            ...
            offset=0, inverted=d_inverted, #dont double count
            capture_width_ns=total_period_ns,
        ...
        timeout_time = (pulse_width_ns + u_to_d_delay + pulse_width_ns + d_to_u_delay - 100) * 1e-9
        ...
        if scope.wait_for_trigger(timeout=5):
            ...
            metadata = {
                ...
                'd_to_u_delay_s': d_to_u_delay*1e-9,
                'u_to_u_delay': u_to_u_delay,
                'polarity': polarity,
                'base_offset': base_offset,
                'capture_width_ns': capture_width_ns,
                'record_length': record_length,
                'timeout_time_us': timeout_time * 1e6
            }
